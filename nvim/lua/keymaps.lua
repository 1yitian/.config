...
map(nvo,	'f',			'e',			'to the [e]nd of the word',				true)
map(nvo,	'F',			'E',			'to the [E]nd of the word',				true)

map(nvo,	'j',			'E',			'to the [E]nd of the word',				true)
map(nvo,	'F',			'E',			'to the [E]nd of the word',				true)
...
map(n,		'<leader>wq',	':wq<CR>',		'[w]rite and [q]uit current buffer')
map(n,		'<leader>t',	':%s/    /	/g','<space> to <tab>')
